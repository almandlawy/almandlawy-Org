...
export default function FloatingConversionBar({
  ...
  return (
    <>
      <div
        className="fixed bottom-0 inset-x-0 z-40 md:hidden border-t border-soft-border bg-brand-card/95 backdrop-blur-md px-2 py-2 safe-area-pb"
        ...
        <div className="flex gap-1.5 max-w-lg mx-auto">
          {onOpenAccount && (
            <button
              ...
              className={`flex flex-col items-center justify-center min-w-[64px] min-h-[48px] px-2 py-1.5 rounded-lg font-mono text-[9px] font-bold uppercase tracking-wide ${
                isLoggedIn
                  ? "bg-gold-base/20 text-gold-dark border border-gold-base/30"
                  : "bg-brand-section text-text-charcoal border border-soft-border"
              }`}
              aria-label={isLoggedIn ? (isAr ? "حسابي" : "My account") : isAr ? "تسجيل الدخول" : "Sign in"}
            >
              <AccountIcon size={16} />
              <span className="mt-0.5 leading-none">{isLoggedIn ? (isAr ? "حسابي" : "Account") : isAr ? "دخول" : "Login"}</span>
            </button>
          )}
          <button
            type="button"
            onClick={onOpenQuote}
            className="flex-1 min-h-[48px] px-2 py-2 bg-gold-base hover:bg-gold-dark text-text-charcoal font-mono text-[10px] font-bold uppercase tracking-wider rounded-lg flex flex-col items-center justify-center gap-0.5"
          >
            <FileText size={16} />
            <span>{isAr ? "طلب عرض" : "Get Quote"}</span>
          </button>
          <a
            href={href}
            target="_blank"
            ...
            className="flex-1 min-h-[48px] px-2 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-mono text-[10px] font-bold uppercase tracking-wider rounded-lg flex flex-col items-center justify-center gap-0.5"
